...
gold <- read_excel("D:/R-code/allgold1.xlsx", 
...
library(xts)
dates <- gold$Date
gold <- xts(x=gold[2], order.by=dates)
...
#getSymbols.yahoo("EURUSD=X", env = EURUSD, from = as.Date("1999-01-04"), to = as.Date("2019-05-17"))
#getSymbols.yahoo("GBPUSD=X", env = GBPUSD, from = as.Date("1999-01-04"), to = as.Date("2019-05-17"))
#getSymbols.yahoo("AUDUSD=X", env = AUDUSD, from = as.Date("1999-01-04"), to = as.Date("2019-05-17"))
#getSymbols.yahoo("CADUSD=X", env = CADUSD, from = as.Date("1999-01-04"), to = as.Date("2019-05-17"))
#getSymbols.yahoo("JPY=X", env = USDJPY, from = as.Date("1999-01-04"), to = as.Date("2019-05-17"))
#getSymbols.yahoo("CNY=X", env = USDCNY, from = as.Date("1999-01-04"), to = as.Date("2019-05-17"))
#getSymbols.yahoo("INR=X", env = USDINR, from = as.Date("1999-01-04"), to = as.Date("2019-05-17"))
currency <- read_excel("D:/R-code/exdata1.xlsx", 
...
# EUR
#eurusd <- currency["EURUSD"]# EURUSD$`EURUSD=X`$`EURUSD=X.Close`
dates <- currency$Date
eurusd <- xts(x=currency["EURUSD"], order.by=dates)
# GBP
gbpusd <- currency["GBPUSD"]#GBPUSD$`GBPUSD=X`$`GBPUSD=X.Close`
dates <- currency$Date
gbpusd <- xts(x=currency["GBPUSD"], order.by=dates)
# AUD
audusd <- currency["AUDUSD"]#AUDUSD$`AUDUSD=X`$`AUDUSD=X.Close`
dates <- currency$Date
audusd <- xts(x=currency["AUDUSD"], order.by=dates)

# CAD
currency["CADUSD"] <- 1/currency["USDCAD"]#CADUSD$`CADUSD=X`$`CADUSD=X.Close`
dates <- currency$Date
cadusd <- xts(x=1/currency["USDCAD"], order.by=dates)
# JPY
#currency["JPYUSD"] <- 1/currency["USDJPY"]
#usdjpy <- currency["USDJPY"]#USDJPY$`JPY=X`$`JPY=X.Close`
#jpyusd <- currency["JPYUSD"]
dates <- currency$Date
jpyusd <- xts(x=1/currency["USDJPY"], order.by=dates)

# CNY
#currency["CNYUSD"] <-1/currency["USDCNY"]#USDCNY$`CNY=X`$`CNY=X.Close`
#cnyusd <- currency["CNYUSD"]
#jpyusd <- currency["JPYUSD"]
dates <- currency$Date
cnyusd <- xts(x=1/currency["USDCNY"], order.by=dates)

#dates <- currency$Date
#currency1 <- xts(x=currency["EURUSD","GBPUSD","AUDUSD","CADUSD","JPYUSD","CNYUSD"], order.by=dates)
#EXP <- currency[,("EURUSD","GBPUSD","AUDUSD","CADUSD","JPYUSD","CNYUSD")]
...
DF <- na.omit(DF)
colnames(DF) <- c("EUR","GBP","AUD","CAD","JPY","CNY","G_USD")
#DF[160,3] <- DF[159,3]
#DF[1320,6] <- DF[1319,6]
#write.table(DF,"D:/R-code/DF1.csv",row.names=FALSE,col.names=TRUE,sep=",")
# create differenced logged return
# log for gold price
lg_gusd <- log(DF$G_USD)
lg_eur <- log(DF$EUR)
lg_gbp <- log(DF$GBP)
lg_jpy <- log(DF$JPY)
lg_cny <- log(DF$CNY)
...
#leur <- log(eurusd$`EURUSD=X.Close`)
#lgbp <- log(gbpusd$`GBPUSD=X.Close`)
#laud <- log(audusd$`AUDUSD=X.Close`)
#lcad <- log(cadusd$`CADUSD=X.Close`)
#ljpy <- log(jpyusd$`JPY=X.Close`)
#lcny <- log(cnyusd$`CNY=X.Close`)
#linr <- log(inrusd$`INR=X.Close`)

# create difference logged return
# diff for gold price and exchange rate
dg_usd <- diff(lg_gusd)
dl_eur <- diff(lg_eur)
dl_gbp <- diff(lg_gbp)
dl_jpy <- diff(lg_jpy)
...
exdata <- merge(dg_usd, dl_eur, join='right')
exdata <- merge(exdata, dl_gbp, join='right')
exdata <- merge(exdata, dl_jpy, join='right')
exdata <- merge(exdata, dl_cny, join='right')
exdata <- merge(exdata, dl_cad, join='right')
exdata <- merge(exdata, dl_aud, join='right')
...
colnames(exdata) <- c("GUSD","eur","gbp","jpy","cny","cad","aud")
